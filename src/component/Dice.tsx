...
const D6 = ({ id, value = '', size = 50, style = {}, isSelected = false, onClick }: { id: string; value?: string; size?: number; style?: React.CSSProperties; isSelected?: boolean; onClick: () => void }) => {
    return (
        <div
            id={id}
            className={`dice ${value === '' ? 'shaking' : 'dice6'} ${isSelected ? 'selected' : ''}`}
            style={{
                width: size,
                height: size,
                fontSize: size * 0.5,
...
export const CustomDice = ({ id, value = "", type = 6, size = 50, style = {}, isSelected = false, onClick }: { id: string; value?: string; type?: number; size?: number; style?: React.CSSProperties; isSelected?: boolean; onClick: () => void }) => {
    ...
    return (
        <div
            id={id}
            className={`dice ${value === '' ? 'shaking6' : 'dice6'} ${isSelected ? 'selected' : ''}`}
            style={{
                width: size,
                height: size,
                fontSize: size * 0.4,
